...
- (void)commonInit
{
    ...
    
    {
        ...
        UIButton *createEvent = [UIButton buttonWithType: UIButtonTypeCustom];
        [createEvent setImage:[UIImage imageNamed:@"plus"] forState:UIControlStateNormal];
        createEvent.frame = CGRectMake(320, 25, 25, 25);
        createEvent.tintColor = [UIColor whiteColor];
        //[addProject setTitle:@"Show View" forState:UIControlStateNormal];
        [createEvent addTarget:self action:@selector(addProjectPressed) forControlEvents:UIControlEventTouchUpInside];
        
        UIButton *settings = [UIButton buttonWithType: UIButtonTypeCustom];
        [settings setImage:[UIImage imageNamed:@"settings_icon-1"] forState:UIControlStateNormal];
        settings.frame = CGRectMake(30, 25, 25, 25);
        settings.tintColor = [UIColor whiteColor];
        //[addProject setTitle:@"Show View" forState:UIControlStateNormal];
        [settings addTarget:self action:@selector(showSettings) forControlEvents:UIControlEventTouchUpInside];
        
        [self addSubview:createEvent];
        [self addSubview:settings];
    }
}

-(void)addProjectPressed
{
    [[NSNotificationCenter defaultCenter] postNotificationName: SHOWCREATEEVENT object:self];
}
