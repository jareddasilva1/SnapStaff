...
- (UIView *)buildMenuItemView
{
    ...
    label.font = [UIFont fontWithName:@"OpenSansRegular" size:20];
...
- (void)prepareMenuItemView:(UIView *)menuItemView date:(NSDate *)date
{
    ...
    if(date){
        ...
        NSDateFormatter *yearOnly = [[NSDateFormatter alloc] init];
        
        [yearOnly setDateFormat:@"YYYY"];
        
        text = [NSString stringWithFormat:@"%@ %@",[[dateFormatter standaloneMonthSymbols][currentMonthIndex - 1] capitalizedString],[yearOnly stringFromDate:date] ];
    }
